...
// FreshRSS can load main.js first, parse the context, and dispatch this event
// while the document is still loading. The extension then attaches its one-shot
// listener too late and remains inert in the current implementation.
const document = {
	readyState: 'loading',
...
// The event was already dispatched by main.js before this extension script ran.
...
setTimeout(() => {
	assert.equal(beacons.length, 1, 'initialization must survive a context event that fired before the script');
	...
	console.log('FreshRSS global-context initialization race passed.');
}, 350);
